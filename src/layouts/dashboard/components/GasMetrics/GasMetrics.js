...
const ADDRESS = "0xE11D08e4EA85dc79d63020d99f02f659B17F36DB";
...
const GasMetricsDashboard = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const validTxs = txData.result.filter(
        ...
        const networkAvgPrice =
          gasPriceData.status === "1" ? parseFloat(gasPriceData.result.ProposeGasPrice) || 0 : 0;

        // Calculate metrics
        const metrics = calculateMetrics(validTxs, networkAvgPrice, ethPrice, data.timeframe);
  ...
  const calculateMetrics = (transactions, networkAvgPrice, ethPrice, timeframe) => {
    ...
    const optimizedTxs = recentTxs.filter(
      (tx) => parseInt(tx.gasPrice) / 1e9 < networkAvgPrice
    ).length;
  ...
  const calculatePerformanceScore = (txs, avgPrice, networkAvg) => {
    ...
    const lowPriceRatio =
      txs.filter((tx) => parseInt(tx.gasPrice) / 1e9 < networkAvg * 0.8).length / txs.length;
    ...
    >
      <VuiBox sx={{ width: "100%", p: 3 }}>
          ...
          {[
            ...
            <Box
              ...
              sx={{
                ...
                borderRadius: "8px",
                cursor: "pointer",
                backgroundColor: data.timeframe === option.value ? "#0075FF" : "#22234B",
                color: data.timeframe === option.value ? "white" : "text.secondary",
                "&:hover": {
                  backgroundColor: data.timeframe === option.value ? "#0065D1" : "#2D2E5F",
                },
              }}
            >
              {option.label}
        ...
        <Grid container spacing={3}>
          {/* Commented out Gas Performance card */}
          ...
          <Grid item xs={12} md={6}>
            ...
            >
              ...
              <VuiBox>
                ...
                <VuiTypography color="text" variant="caption">
                  ({Math.round((data.metrics.optimizedTxs / data.metrics.totalTxs) * 100)}% of
                  total)
                </VuiTypography>
              </VuiBox>
            </Card>
            ...
            >
              ...
              >
                <CircularProgress
                  ...
                  thickness={6}
                  color={
                    data.metrics.healthScore >= 8
                      ? "success"
                      : data.metrics.healthScore >= 5
                      ? "warning"
                      : "error"
                  }
                  sx={{
              ...
              <VuiTypography color="text" variant="caption" textAlign="center" mb={1}>
                Current Network
              </VuiTypography>
              <VuiTypography color="white" variant="h6" fontWeight="bold" textAlign="center">
        ...
        <VuiBox mt={3}>
          <VuiTypography color="text" variant="caption">
            Analyzing {data.metrics.totalTxs} transactions from {ADDRESS.slice(0, 6)}...
            {ADDRESS.slice(-4)} over last {data.timeframe} days
          </VuiTypography>
        </VuiBox>
      </VuiBox>
    </Card>
