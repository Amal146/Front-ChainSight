...
const ADDRESS = "0xE11D08e4EA85dc79d63020d99f02f659B17F36DB"; // USDT contract
const API_KEY = "V8RHS7P2YNSAHUY92CXVANVQK8MIYK95UQ";

// Utility functions moved to the top level
const calculateEfficiencyScore = (gasMetrics, recentTransactions) => {
  if (recentTransactions.length === 0 || gasMetrics.networkAverageGasPrice === 0) return 0;

  const avgUserPrice =
    gasMetrics.gasPriceTrends.reduce((sum, day) => sum + day.avgPrice, 0) /
    gasMetrics.gasPriceTrends.length;

  // Score from 0-10 based on how much better than network average
  let score = 10 * (1 - avgUserPrice / gasMetrics.networkAverageGasPrice);
...
const calculatePotentialSavings = (gasMetrics, recentTransactions) => {
  if (recentTransactions.length === 0) return 0;

  return recentTransactions.reduce((total, tx) => {
    const optimalPrice = gasMetrics.networkAverageGasPrice * 0.8; // Assuming 20% below average is optimal
    const currentPrice = parseInt(tx.gasPrice) / 1e9;
    const gasUsed = parseInt(tx.gasUsed || tx.gas);

    if (currentPrice > optimalPrice) {
      return total + ((currentPrice - optimalPrice) * gasUsed) / 1e9;
    }
    return total;
  }, 0);
};
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const successfulTxs = txData.result.filter(
          (tx) => tx.isError === "0" && tx.gasPrice && (tx.gasUsed || tx.gas) && tx.timeStamp
        );
      ...
      <Grid container spacing={4} mb={3}>
        ...
        <Grid item xs={6} sm={4}>
          <MetricCard
            ...
            progress={Math.min(
              ...
              gasMetrics.gasPriceTrends.length > 0
                ? (gasMetrics.gasPriceTrends.reduce((sum, day) => sum + day.avgPrice, 0) /
                    (gasMetrics.gasPriceTrends.length * gasMetrics.networkAverageGasPrice)) *
                    100
                : 0
            )}
            disabled={recentTransactions.length === 0}
        ...
        <Grid item xs={6} sm={4}>
          <MetricCard
            ...
            value={`${calculateEfficiencyScore(gasMetrics, recentTransactions).toFixed(1)}/10`}
            secondaryValue={getEfficiencyLabel(
              calculateEfficiencyScore(gasMetrics, recentTransactions)
            )}
            progress={calculateEfficiencyScore(gasMetrics, recentTransactions) * 10}
            color={getEfficiencyColor(calculateEfficiencyScore(gasMetrics, recentTransactions))}
          />
        </Grid>
        ...
        <Grid item xs={6} sm={4}>
          <MetricCard
            ...
            value={`${calculatePotentialSavings(gasMetrics, recentTransactions).toFixed(6)} ETH`}
            secondaryValue={`$${(
              calculatePotentialSavings(gasMetrics, recentTransactions) * ethPrice
            ).toFixed(2)}`}
            progress={Math.min(
              100,
              calculatePotentialSavings(gasMetrics, recentTransactions) * 100
            )}
          />
        </Grid>
      </Grid>
              ...
              <Typography
                ...
                color={
                  isNaN(gasMetrics.networkAverageComparison)
                    ? "#ffffff"
                    : gasMetrics.networkAverageComparison > 0
                    ? "#ff6b6b"
                    : "#51cf66"
                }
                fontWeight="bold"
                ...
                {recentTransactions.length > 0
                  ? `${
                      !isNaN(gasMetrics.networkAverageComparison)
                        ? `${
                            gasMetrics.networkAverageComparison > 0 ? "+" : ""
                          }${gasMetrics.networkAverageComparison.toFixed(2)}%`
                        : "N/A"
                    }`
                  : "No recent data"}{" "}
                  ...
                  <LineChart data={lineChartData}>
                    ...
                    <Tooltip
                      ...
                      formatter={(value, name) => [
                        `${value} ${name.includes("Gwei") ? "Gwei" : "ETH"}`,
                        name,
                      ]}
                    />
        ...
        <Grid item xs={12} lg={5}>
          ...
          >
            ...
            <Typography variant="body2" color="rgba(255,255,255,0.7)" mb={3}>
              ...
              {recentTransactions.length > 0 &&
                ` (${recentTransactions.length} in last ${timeframe} days)`}
            </Typography>
              ...
              {barChartData.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={barChartData}>
                    ...
                    />
                    <Bar dataKey="Daily Gas Spent (ETH)" fill="#4BB543" radius={[4, 4, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              ) : (
                <Box display="flex" justifyContent="center" alignItems="center" height="100%">
...
const MetricCard = ({ icon, title, value, secondaryValue, progress, disabled = false }) => (
  ...
  >
    <Stack direction="row" alignItems="center" spacing={1} mb={1}>
      ...
      <Typography
        variant="body2"
        color={disabled ? "rgba(255,255,255,0.5)" : "rgba(255,255,255,0.7)"}
      >
        {title}
      </Typography>
    </Stack>
